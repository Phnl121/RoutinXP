...
- **Compact:** 40px tall, 16px side padding. Only for "+ Nova tarefa" in the Tarefas title row, where it sits beside the page title and opens the new-task dialog. On desktop, Painel and Perfil have no create button; the side menu leads back to Tarefas.
...
- **FAB:** at 60rem and below, a 56px Action Violet circle with a plus icon, fixed 1.25rem from the bottom-right (safe-area aware), carrying the FAB glow. It replaces the Tarefas title row's "+ Nova tarefa" and appears on every page: the shell renders it everywhere except Tarefas, which renders its own that opens the dialog in place.
