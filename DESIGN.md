...
- Money is neutral: sign and weight carry direction, never green or red; transfers recede to Muted.
...
- **Ink** (`ink`): primary text; today and overdue due dates; the fill of the neutral ERRO tag; the selected swatch ring; the chart's one direct value label; the "Mais concluída" line; the track and border of a switch that is on; the "Suspensa" state pill's text; every money figure on Financeiro except a transfer's; the "A revisar" detail line of an uncategorized entry; the largest bar in Para onde foi, the filled Entradas bar of the six-month chart and its legend key, the chosen month's name under that chart.
- **Muted** (`muted`): secondary text, labels, inactive tabs and segments, side-menu items at rest, icon buttons at rest, the shell strips' leading icons (clock, no connection, download), the iOS install step numbers, hints, "faltam n XP", category names, counts, future due dates, chart axis and cap labels, timeline times, the Excluir label, and a completed row's or timeline item's "+0 XP" and "…" (XP pending); state pill text, the knob of a switch that is off, "você" and "último acesso" in the account list; a Financeiro transfer's value, the day result, the account type line and "sem entradas"; the scoreboard's comparison lines, a percentage share, every Para onde foi bar except the largest, the 1.5px ring of the outlined Saídas bar, the Resultado do mês chevron and its opened category lines, a scheduled entry's description and value; on Gastos fixos, a paid charge's name and value and its "paga" check, a paused or paid-off registration's name and value, the registration's right-hand extra ("R$ 670,80/ano", "próxima: 16 set", "pausada").
...
- **Header:** a three-column grid (1fr | auto | 1fr, 0.75rem by 1rem gap). The Page Title "Financeiro" sits at the left. The month selector sits in the center: two 2.5rem chevron buttons (Muted, 10px radius, Panel Two and Ink on hover; 2.75rem at 60rem and below) around the month name ("Setembro de 2026", 1.0625rem at 800, min 11.5rem, centered, announced politely). The actions sit at the right, 1.25rem apart: "Categorias" as a link button, then the primary "+ Lançamento".
- **Scoreboard:** see Financeiro under Components.
- **Grid:** entries at 1.65fr and the side column at 1fr, 1.25rem gap, top-aligned. The side column stacks 1.25rem apart: Contas e cartões, the Gastos fixos card (when any exists), Para onde foi, Resultado do mês, Últimos 6 meses.

Container steps: at 66rem and below the header puts title and actions on the first row and the month selector on a second row, left-aligned. At 58rem and below the grid becomes one column and the side column dissolves into it (`display: contents`): Contas e cartões comes first, because the balance is the first question, then the entries, then the Gastos fixos card and the summary panels; the scoreboard gains "Ver resumo do mês", a link button under the cells (0 1.5rem 0.9rem margin) that jumps to Para onde foi, and it shows only in this one-column layout. At 44rem and below the month selector spreads across the full width, the scoreboard becomes 2 by 2, the type segments take a full row and split it, and the selects share a row (9rem basis). At 30rem and below each select takes its own row and the Resultado do mês columns narrow to 2.5rem and 6.5rem. The first run (no account yet) replaces scoreboard and grid with a single panel.
...
- **Money charts:** Financeiro's Para onde foi and Últimos 6 meses do not use green; they follow the Neutral Money Rule (see Financeiro).
...
### Financeiro
The month on one page (user decision, 2026-09-13), reached from the side menu's Finanças section (wallet icon). For its grid and container steps, see Layout. Every figure is tabular, in reais formatted pt-BR, and follows the Neutral Money Rule.
- **Scoreboard:** one Panel with no padding, four equal cells (Entradas, Saídas, Resultado, Poupado) divided by 1px Rules. A cell pads 1.1rem by 1.5rem (0.9rem by 1.1rem when 2 by 2) and holds its Label caption over the Stat Figure, 0.4rem apart, never wrapping. Resultado carries its sign; Poupado is a percentage, or "sem entradas" at 0.9375rem and 600 in Muted. In the 2 by 2 layout the Rules follow the cells. Under each figure a comparison line (0.75rem, 600, Muted, tabular) compares with the previous month: Entradas and Saídas as a signed percentage ("+19% sobre agosto"), Resultado as a difference ("R$ 88,30 a mais que em agosto"), Poupado in points ("−7 p.p. sobre agosto"), "igual a agosto" when equal. While the month is under way it compares with the previous month up to the same day ("sobre agosto até o dia 13"). With no history in the previous month there is no line; cells top-align so the figures stay on one row.
- **Filters:** one wrapping row 0.6rem apart: the search field with the magnifier inside (basis 13rem), the in-flow type segments (Todos, Entradas, Saídas, Transferências), and two selects (Todas as categorias, with "A revisar" as an option, and Todas as contas). With a filter on, a 0.875rem Muted `role="status"` line counts the entries with their inflow and outflow sums, with "Limpar filtros" as a link button at the right.
- **Entries by day:** each day is a Label heading (1rem above, 0.45rem below, 0.75rem inset) with the day ("DOM, 13 DE SET") at the left and its result at the right, then the day's rows in a hairline list: Panel fill, 1px Rule border, 10px radius, no shadow, 1px Rules between rows. The list is not a card and the rows are not cards.
- **Entry row:** a button, min 3.5rem, 0.55rem by 1rem padding, 0.85rem gaps, Panel Two on hover. Three columns: the 0.625rem category dot (a hollow 2px Check Ring ring for a transfer or an entry "A revisar"); the description at 700 over the detail line (0.8125rem Muted: "Lazer · Cartão Nubank", or "Nubank → Poupança" for a transfer), both ellipsized; and the value at 800. Only inflows show "+"; outflows show "−". It opens the entry dialog.
- **Scheduled entries:** an entry dated after today (a future installment) stays in its day, but the day heading reads "DOM, 20 DE SET · AGENDADO", its description and value are Muted, and it counts in neither the scoreboard, the filter sums nor the summaries; the day result in the heading is the scheduled sum.
...
- **Contas e cartões:** a Panel padded 1.5rem (1.25rem at 60rem and below), titled in Label style. Each account is a row button (min 3.25rem, 0.45rem by 0.75rem padding, 8px radius, Panel Two on hover): the name at 700 over its type in Meta Muted ("Cartão de crédito · vence dia 10"), and the balance at 800 at the right. A card in debt shows its bill as "fatura R$ 857,20", the word in Meta Muted. Below, over a 1px Rule, "SALDO TOTAL" in Label style and the total at 1.25rem and 800; then "+ Nova conta" as a Soft Violet text action, with "n arquivadas" in Meta Muted at the right when there are any.
...
- **Para onde foi:** the month's outflows by category, largest first. Under the title, one sentence at 0.875rem and 600 in Ink ("Onde mais gastou: Moradia, 62% das saídas."). Each category is a row button (0.5rem by 0.75rem padding, 8px radius, Panel Two on hover): the 0.625rem category dot and the name at 600 (ellipsized), the share (0.75rem, 600, Muted, right-aligned) and the value at 800 on one line; below it, a full-width 0.5rem Panel Two track (Rule on hover) holding the bar, square at the start and 4px round at the end. Every bar measures on the same full-width track, scaled to the largest category (never below 1.5%). Bars are Muted and the largest is Ink; the category color stays on the dot. "A revisar" takes the hollow ring. Tapping a row sets the category filter, resets the type to Todos and scrolls to the entries. Empty: "Nenhuma saída neste mês." in Meta Muted.
- **Resultado do mês:** the personal income statement, a hairline list: Receitas, Despesas fixas, Despesas variáveis, Assinaturas, and A revisar only when it holds something, then Resultado. A row is a three-column grid with fixed columns (the label, the share in 2.75rem, the value in 7.25rem), so shares and values align down the list like a statement; min 2.75rem, 0.35rem by 0.75rem padding, 8px radius, 1px Rules between rows. The label is 600; the share is 0.75rem, 600, Muted, a percentage of the month's receitas (none on Receitas); the value is 800 and signed (Receitas "+", expenses "−"). A row with categories is a button (Panel Two on hover, `aria-expanded`) led by a Muted 2px chevron that turns from right to down; it opens its categories beneath, indented 2rem, each a 2rem line at 0.875rem in Muted with a 0.5rem dot and the value. Rows without categories indent their label to align with the chevron rows. The Resultado row sits over a Rule Strong divider, its label at 800 and its value at 1.125rem.
- **Últimos 6 meses:** entries against exits for the six months ending at the chosen month. The header carries a legend at the right (0.75rem, 600, Muted: a 0.625rem key with 2px corners per series). A 9rem plotting area over a 1px Rule baseline holds one column per month, 0.35rem apart; each column is a button (8px top corners, Panel Two on hover and when chosen, `aria-pressed`) holding two bars 2px apart, min(1rem, 40%) wide with 4px top corners, scaled to the largest value of either series: **Entradas** filled Ink, **Saídas** Panel Two with a 1.5px Muted inset ring. Series differ by shape, not color. Short month names run beneath (0.75rem Muted; the chosen month Ink at 700). No bar is labeled; hovering, focusing or tapping a column chooses it, and its figures show as text in the figcaption, announced politely: the month name at 0.875rem in Ink on its own line, then "Entradas R$ …", "Saídas R$ …" and "Resultado +R$ …" at 0.8125rem Muted. Each column's label also carries its figures.
- **First run:** with no account yet, a Panel (max 36rem, 1.75rem padding) holds a 1.25rem/800 title "Comece pela sua primeira conta", one Muted sentence and the primary "+ Cadastrar conta". The header keeps "Categorias" but drops "+ Lançamento", and there is no FAB.
- **Entry dialog:** the standard Dialog. The in-flow segments choose Saída, Entrada or Transferência; then Valor (money field) and Data as a pair; Descrição; Categoria, only the categories of the chosen type (absent for a transfer); the account ("Saiu de", "Entrou em", or "De" and "Para" as a pair). A Meta hint explains that paying a card bill is a transfer. Editing adds "Excluir lançamento" as the destructive text action; deleting shows the undo toast "Lançamento excluído.".
- **Account dialog:** the standard Dialog. Nome and Tipo as a pair, then the money field (Saldo atual, or Fatura em aberto for a card) with a Meta hint, the neutral checkbox "Saldo negativo (cheque especial)" or, for a card, Fecha dia and Vence dia as a pair of selects. Editing adds the "Arquivar conta" checkbox and "Excluir conta"; an account with entries can't be deleted, and the notice says to archive it.
...
- **Próximos 30 dias:** a Panel padded 1.5rem, titled in Label style. Its groups, each under a Label heading (0.5rem above, 0.35rem below): **Vencidas** first (unpaid charges already due), then one group per day ("QUA, 16 DE SET"), then **Pagas este mês** (charges already paid whose day has passed). A charge row is not a button: min 3.25rem, 0.45rem vertical padding, 1px Rules between rows in a group. At the left, the name at 700 over the detail line (0.8125rem Muted: "Conta fixa · valor aproximado · Nubank", "parcela 2 de 6 · Cartão Nubank"), both ellipsized. At the right, the value at 800 over a state line (0.8125rem, 0.6rem gaps): unpaid, the task Deadline Badge ("atrasada · 16 ago", "16 set") and "Pagar" as a link button; paid, "paga" at 700 in Muted with a 0.75rem check, then "Desfazer", while name and value turn Muted; an installment, "lançada" in Meta Muted with no action, because its entries already exist. Paying a fixed value records it today and shows the undo toast "Conta de luz marcada como paga."; a variable value opens the Pagar dialog. Empty: "Nada vence nos próximos 30 dias." in Meta Muted.
- **Registrations by type:** one Panel per type that has items (Assinaturas, Compras parceladas, Contas fixas, Outros), padded 1.5rem, the Label title at the left and the monthly subtotal at the right ("R$ 98,53/mês", 0.875rem at 800, tabular). Each item is a row button in the account-row recipe (min 3.25rem, 0.45rem by 0.75rem padding, 8px radius, Panel Two on hover), active items first: the name at 700 over the detail in Meta Muted ("todo dia 6 · Cartão Nubank", "todo ano em 3 nov", "7 de 12 · faltam R$ 2.082,50"), and at the right the value at 800 over an extra line (0.75rem, 600, Muted: "R$ 670,80/ano" for a subscription, "próxima: seg, 5 de out" for a bill, the card for an installment, "pausada" or "quitada"). A paused or paid-off item stays listed with its name and value in Muted. It opens the Gasto fixo dialog.
...
- **Do** draw charts in single-hue green with 4px rounded data ends, a neutral scale reference, one direct label on the extreme value, a hover tooltip, and a visually hidden table with the same data. Money charts are the exception: neutral bars (Muted, the largest in Ink) and series told apart by shape, per the Neutral Money Rule.
...
- **Do** show money as signed tabular figures in Ink, drop transfers to Muted, and mark an uncategorized entry with a hollow ring and "A revisar".
...
- **Don't** color money green or red, tint a negative balance or a month-over-month change; sign and weight carry direction.
