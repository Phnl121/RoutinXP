...
**The Dot-Only Category Rule.** User-chosen category colors are data. They appear as an 8px round dot beside the category name (rail, chips, row category line, Lista group headings, the Painel's category bars and timeline items), never as fills, text color, bars or backgrounds, and never without the name nearby. Two exceptions: the color picker, where the swatch itself is the choice and fills a 36px circle; and the task row's 2px top border in its category color (user request, 2026-09-11), which marks where one task ends and the next begins, in Lista and Quadro alike. On completed rows the top border fades to 40% of the category color, receding with the rest of the row. **Tags** follow the same rule, stricter, with their own mark: a **ring** (9px, a 2px inset stroke in the tag color, hollow), never a filled dot, so a tag can never be read as a category even when both use the same color. Outside the opened task a tag is only its ring, placed on the row's meta line after the category; its name shows only inside the task dialog and on the Perfil tag list (and in the row's tooltip and accessible name).
...
2. A text button (opens edit): Row Title, then a meta line in Meta muted: the category (8px dot and name, only when the view mixes categories), then the **tag rings** (0.3rem apart, no names). The tag names join the button's accessible name ("Editar …, Tags: …") and its tooltip; the rings are `aria-hidden`. The meta line is omitted when it would hold only the date. The description never shows in the row.
...
- **Don't** fill, border or tint surfaces or chart bars with category colors. The one border exception is the task row's 2px top rule (user override, 2026-09-11), faded to 40% on completed rows.
- **Don't** draw a tag as a filled dot: filled dots are categories, rings are tags.
