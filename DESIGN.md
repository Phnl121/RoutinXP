...
colors:
  ...
  on-prazo: "#1d1407"
  entrada: "#2dd4bf"
  entrada-wash: "rgb(45 212 191 / 0.14)"
  saida: "#fb7a6a"
  saida-wash: "rgb(251 122 106 / 0.14)"
  transferencia: "#7cc6f5"
  transferencia-wash: "rgb(124 198 245 / 0.14)"
  purple: "#7c3aed"
  purple-hover: "#8b5cf6"
  purple-soft: "#c4b5fd"
...
components:
  ...
  money-field:
    backgroundColor: "{colors.ground}"
    textColor: "{colors.ink}"
    typography: "{typography.money-entry}"
    rounded: "{rounded.sm}"
    height: "48px"
  fin-account-row:
    backgroundColor: "transparent"
...
- Category color is data: an 8px dot beside the category name, plus the solid cover band on top of every task card (the color picker swatch is the only other fill). On the money pages a category is a tinted 2rem tile and a tinted name. Tags are rectangular (labels, bars, swatches), never dots.
...
- Charts are single-hue green magnitude, directly labeled once, with a table for screen readers. Money charts take the direction and category colors instead.
- Money has its own direction hues, always beside the sign: Entrada teal in, Saída coral out, Transferência sky between one's own accounts. Neither is Level Green nor Deadline Rose.
- Motion is one ease-out curve; reduced-motion collapses it to near zero.

## Colors

A cool, near-black neutral stack with two saturated accents whose roles never overlap. The two accent values are the logo's colors (public/marca), so the brand mark and the interface speak the same violet and green. Translucent effect tokens derive from the accents so glows, washes and halos stay themeable. Money adds three direction hues of its own, which are not accents.
...
Money direction hues (user request, 2026-09-14). They are not accents either: they appear only on money, beside a sign or a legend (see the Money Direction Rule).
- **Entrada Teal** (`entrada`): money coming in. An inflow's value, the scoreboard's Entradas, a positive Resultado, day result or Resultado do mês value, Receitas, and the Entradas bars and legend key of Últimos 6 meses. A teal deliberately apart from Level Green.
- **Saída Coral** (`saida`): money going out. An outflow's value, the scoreboard's Saídas, a negative Resultado, day result or Resultado do mês value, a negative account balance and a card's open bill, every unpaid Gastos fixos value, and the Saídas bars and legend key of Últimos 6 meses. A coral deliberately apart from Deadline Rose.
- **Transfer Sky** (`transferencia`): a transfer's value (at 700) and the dot of its category tile.
- **Direction Washes** (`entrada-wash`, `saida-wash`, `transferencia-wash`): the three hues at 14%. Transfer Wash fills a transfer's category tile; the other two are defined for parity and no component reads them yet.
...
- **Ground** (`ground`): page background, input wells, the segmented-filter well (and the in-flow segments' well), the temporary-password well, the knob of a switch that is on, the duplicate notice and the Juntar contas box inside the Financeiro dialogs.
- **Panel** (`panel`): raised containers (form panel, demo panel, Kanban columns, task cards in Lista, Calendário Dia and Linha do tempo, the filter panel, Painel cards, Perfil panels, dialog, account menu, the level pill, the shell strips (offline strip, streak and deadline reminders, install invite), the Financeiro scoreboard, entry lists, Contas e cartões panel, the Bancos conectados panel, the Gastos fixos card and the three summary panels; on Gastos fixos, the scoreboard, Próximos 30 dias and the type panels).
- **Panel Two** (`panel-2`): third tier. XP track and skeleton, task card hover (and the card body inside Kanban columns), side-menu item hover and current page, user-card hover, a selected filter chip, the active segment, notice and toast backgrounds, the row's Excluir overlay, avatar fill, the chart column hover, the chart tooltip, the category-bar track, a state pill's fill, a switch track that is off, an account row on hover, a Financeiro entry row or account row on hover, the month arrows on hover; a Para onde foi category row, an expandable Resultado do mês row, a six-month column (on hover and when chosen) and a Gastos fixos registration row on hover; the track under a Para onde foi bar (Rule on hover).
- **Rule** (`rule`): 1px panel borders, row dividers, tab baseline, input border at rest, unselected chip border, the streak divider, the side menu's right edge, the shell strips' border, the chart baseline, the 2px timeline connector, the hairlines between account rows, Registro items and switch rows, the Financeiro scoreboard's cell dividers, entry-list borders and hairlines, and the rule above Saldo total; the hairlines between Resultado do mês rows, the six-month chart baseline, and the hairlines between charges on Gastos fixos.
- **Rule Strong** (`rule-strong`): input hover border, notice and toast border, the current side-menu item's inset outline, a selected filter chip's border, the dashed "+ Nova coluna" Kanban slot, the dashed daily-cap line on the XP chart, the chart tooltip border, the share-glyph chip in the iOS install steps, strike-through on completed titles, the destructive text action's underline, the toast time bar, scrollbar thumb, the state pill outline, the border of a switch that is off, the temporary-password well's border, the dashed empty-month box on Financeiro, the divider above the Resultado row of Resultado do mês, the border of the "a revisar" strip, of the duplicate notice and of the Juntar contas box, and the 1.5px outline of a hollow category tile.
- **Check Ring** (`check-ring`): the 2px ring of an unchecked completion circle. It exists because Rule Strong fell below 3:1 on Panel. Check Ring holds at least 3:1 (WCAG 1.4.11 non-text contrast) on every surface a card can have: 4.2:1 on Panel, 3.7:1 on Panel Two (hovered row, Kanban card) and 3.3:1 on Rule (hovered Kanban card). It also draws the 2px ring inside the hollow category tile of an entry with no category (Financeiro, Para onde foi, Gastos fixos).
...
- **Ink** (`ink`): primary text; today and overdue due dates; the fill of the neutral ERRO tag; the selected swatch ring; the chart's one direct value label; the "Mais concluída" line; the track and border of a switch that is on; the "Suspensa" state pill's text; the money figures that carry no direction (Poupado, Saldo total, a zero result, the Gastos fixos subtotals and scoreboard); the "A revisar" detail line of an uncategorized entry and its category word; the "pendente" and "possível duplicata" marks; the "a revisar" strip's sentence; a bank's error line and the Bancos conectados status line on failure; the chosen month's name under the six-month chart.
- **Muted** (`muted`): secondary text, labels, inactive tabs and segments, side-menu items at rest, icon buttons at rest, the shell strips' leading icons (clock, no connection, download), the iOS install step numbers, hints, "faltam n XP", category names, counts, future due dates, chart axis and cap labels, timeline times, the Excluir label, and a completed row's or timeline item's "+0 XP" and "…" (XP pending); state pill text, the knob of a switch that is off, "você" and "último acesso" in the account list; the account type line and "sem entradas"; the scoreboard's comparison lines, a percentage share, a Para onde foi bar with no category color, the Resultado do mês chevron and its opened category lines, a scheduled entry's description and value, a bank's last-read line; on Gastos fixos, a paid charge's name and value and its "paga" check, a paused or paid-off registration's name and value, the registration's right-hand extra ("R$ 670,80/ano", "próxima: 16 set", "pausada").

### Light Theme
User request, 2026-09-14. Dark stays the default; Perfil > Aparência offers Sistema, Claro and Escuro (saved per browser, applied in index.html before first paint), and the account menu has a one-tap Tema claro/escuro. The light theme is the opposite of the dark one with the same soul: a cool paper Ground (#f3f2f8) with a whisper of lavender, white Panels that rise by shadow instead of by lightness, Panel Two #f1f0f6, Rule #e4e2ec, Ink #17161f, Muted #5c596b. Every color that becomes text takes a deeper version of its hue, at least 4.5:1 on white and Panel Two: Entrada #0b7a6d, Saída #c23a28, Transferência #16699f, tom-assinatura #7a56c0, tom-parcelada #3566c8, tom-conta #7d6327, green text (green-texto) #15803d, deadline amber text (prazo-perto-texto) #9a5a0a, links (purple-soft) #6d28d9. Fills keep the brand vivid: the XP bar and level badge stay bright green (#1fb455 with dark on-green), buttons stay #7c3aed with white. The overdue badge is white on #c2365a (on-atrasado). The logo swaps to a version whose "Routin" is Ink. Every surface reads tokens, so both themes share one stylesheet; a new color must be added to both the :root block and the [data-tema='claro'] block in src/index.css.

**Beige theme** (user request, 2026-09-14): a fourth option, Bege, built like the light theme but warm, like a notebook: oat Ground #efe7d8, ivory Panel #fbf7ee, Panel Two #f3ecdf, Rule #e4d9c6, coffee Ink #2b2219, Muted #695d4e, warm amber-brown shadows. Text hues are warmed and deepened to at least 5:1 on Panel Two (Entrada #0a6e61, Saída #b0351f, Transferência #1b5f8c, green-texto #156b37, links #6124c4). It uses the Ink logo. The account menu toggle offers Tema escuro from any light theme and Tema claro from dark; Bege and Sistema are chosen in Perfil.

### Named Rules
**The Two Jobs Rule.** Green means progress and purple means action; neither ever stands in for the other. No green buttons, no purple XP, no third accent. Money has its own direction hues (see the Money Direction Rule), and they are not accents: they live on money figures, money charts and the transfer tile, never on a button, a selection or a page surface, and they never stand in for progress, so money coming in is teal, not Level Green. Purple never marks selection: a current side-menu page, a selected chip, segment or swatch is shown with neutrals (Panel Two, Rule Strong, Ink). The single violet indicator is the active underline-tab bar, which marks the current mode. Green is also never spent on something that earned nothing: a "+0 XP" is Muted, and no flight launches for it. The deadline tints (Tertiary) are status, not a third accent: they stay inside the Deadline Badge and the deadline reminder's clock. Administration takes no accent for state: an account's state pills and its feature switches are neutrals (nothing there is progress, nothing is an alarm), and violet stays on the actions.
...
**The Dot-Only Category Rule.** User-chosen category colors are data. They appear as an 8px round dot beside the category name (filter chips, the Categorias e tags list, row category line, Lista group headings, the Painel's category bars and timeline items, the finance Categorias dialog), never as fills, text color, bars or backgrounds, and never without the name nearby. Two exceptions: the color picker, where the swatch itself is the choice and fills a 36px circle; and the task card's **cover** in its category color (user request, 2026-09-11, after a Trello reference): a solid band across the top of every task card, 0.375rem tall in Lista, Calendário Dia and Linha do tempo, and 0.875rem in Kanban (density pass, 2026-09-11). The card body is Panel in Lista, Dia and Linha do tempo (Panel Two on hover) and Panel Two inside the Kanban columns (Rule on hover), so the card always sits one step above what holds it. Cards are separated by a 0.375rem gap, never wrapped in another panel. On completed cards the cover fades to 40% of the category color, receding with the rest of the card. The narrow-width Calendário month is the other exception: it shows category dots with no names, because the day opens to the full list with names. The money pages are the last exception (user request, 2026-09-14): on Financeiro and Gastos fixos a category is a tinted 2rem tile, its name is tinted, and each Para onde foi bar takes its category color (see the Money Direction Rule). **Tags** have their own shape, always rectangular, so a tag is never read as a category (categories are always round dots). On a task row tags are **labels, Trello-style** (user request, 2026-09-11): a row of 4px-radius labels above the title. Named mode: a 1.25rem label, 0 0.4rem padding, 0.75rem at 700 in Ink on the tag color mixed 48% into Panel (55% on hover, the most that keeps Ink above 4.5:1 on all eight colors), ellipsized at 12rem. Color-only mode: a 2.5rem by 0.5rem bar in the full tag color. Clicking any label switches every label between the two modes (saved per browser); it never opens the task. On touch screens the label is too short to tap reliably, so it ignores taps and the tap opens the task; the switch is a mouse affordance. Completed rows show labels at 55% opacity. In the task dialog and the Perfil list a tag is a 0.75rem swatch with 3px corners beside its name.

**The Budget and Goal Rule.** (Phase 6, 2026-09-14.) A category limit is a full-width row: the category dot and name, the percentage at the right, a 0.5rem pill track filled in the category color, and "R$ 310,90 de R$ 300,00" in Muted below. From 80% the row takes a Deadline Amber wash, its bar, percentage and a written state ("Perto do limite · restam R$ 11,00") turn amber; from 100% the same in coral on Saída Wash ("Passou do limite · R$ 10,90 a mais"). The state is always written, never color alone. Rows sort tightest first. The savings goal card is tinted teal like an inflow tile: the amount kept this month large in Entrada teal (coral if negative), "guardados de R$ 1.140,00" in Muted, a 0.625rem teal track and one sentence ("20% das entradas · Faltam R$ 300,00 para a meta"). Both panels ignore the Controle filters and hide while a filter is on; on a single column they stack Orçamento then Meta before the charts. The due-date strip reuses the reminder strip with the repeat icon in amber (coral when something is overdue). Alert strips on Lançamentos (budget amber or coral, suggestion lavender) sit right under the month selector, above the accounts.

**The Money Direction Rule.** Money has its own three hues (user request, 2026-09-14, replacing the Neutral Money Rule, because a one-color Financeiro read poorly), and color always travels with the sign. **Entrada** teal is deliberately not Level Green and **Saída** coral is deliberately not Deadline Rose, so money never reads as XP or as an overdue alarm; **Transferência** sky marks money moving between one's own accounts. An entry's value is 800 in its direction hue ("+R$ 900,00" teal, "−R$ 89,00" coral); a transfer's value is sky at 700; a scheduled entry's value is Muted. On the scoreboard Entradas is teal and Saídas coral; Resultado, a day's result and every Resultado do mês value take the hue of their sign, and a zero stays Ink. A negative account balance and a card's open bill are coral; Saldo total stays Ink. On Gastos fixos every value is an outflow, so charge and registration values are coral, and Muted once paid, paused or paid off. Poupado, the Gastos fixos subtotals and scoreboard, and the comparisons with the previous month stay neutral: a comparison is a Muted sentence with its sign ("+19% sobre agosto até o dia 13", "R$ 88,30 a mais que em agosto", "−7 p.p. sobre agosto"), never an up or down color. Categories are identity here: every entry row, Gastos fixos row and Para onde foi row leads with a **category tile**, a 2rem square with 8px corners, the category color mixed 22% into transparent, with a solid 0.625rem dot of the color centered. An entry with no category gets a hollow tile (a 1.5px Rule Strong outline around a 2px Check Ring ring) and "A revisar" in Ink, a light request for attention; a transfer gets a Transfer Wash tile with a sky dot. The category name on the detail line is the category color mixed 70% with Ink, so it reads on Panel. Money charts use the same hues: each Para onde foi bar is its category's color (Muted when it has none), and Últimos 6 meses draws Entradas as teal bars and Saídas as coral bars, named in the legend. Green stays with XP and level, violet with the actions ("+ Lançamento", "Regras", "Categorias"). A future-dated entry (a scheduled installment) is listed but "agendado": its description and value drop to Muted, and it stays out of the scoreboard, the comparison, the filter sums, the day result and the summaries.

**Toned Cards (user request, 2026-09-14).** On Financeiro and Gastos fixos the direction hues also tint surfaces while text stays Ink: the scoreboard is four separate cards (Radius Large, Panel lift) filled with a 9% mix of their tone over Panel, a 24% radial glow from the top-right corner and a 38% tone border; the label takes a 55% mix of the tone into Ink. Tones: Entradas teal, Saídas coral, Resultado by sign (Muted at zero), Poupado sky; on Gastos fixos, Por mês coral, Assinaturas por ano lavender (tom-assinatura), Parcelas a pagar blue (tom-parcelada), Pagos este mês teal. Entry rows fill with an 8% mix of their category color (15% on hover), transfers with sky; uncategorized rows stay on Panel so the review stands out. Saldo total and the DRE Resultado row are 12% tone bands with a 30% inset ring; the Bancos conectados card is sky at 7%, the Gastos fixos card coral at 8%. Gastos fixos groups take their type tone (5% fill, 14% corner glow, 32% border) from the tokens tom-assinatura, tom-parcelada, tom-conta (sand, kept apart from Deadline Amber) and tom-outro; each row is a 7% category tint, and an overdue charge is a 12% coral band with a 35% coral ring.
...
- **Stat Figure** (800, 1.5rem, line-height 1.1, tabular): the Painel's streak figures ("6 dias"), set above their Label caption (STREAK ATUAL, RECORDE), and the Financeiro scoreboard's four figures under their Label captions (1.25rem when the page is 44rem wide or less). The Saldo total runs 1.25rem at 800.
...
- **Money Entry** (800, 1.125rem, tabular): the typed value in the money field (Valor, Saldo atual, Fatura em aberto).
...
**Deadline Badge.** The due date on every task card (and on every unpaid charge on Gastos fixos) is a 1.25rem, 5px-radius badge (0 0.4rem padding) with a 0.75rem clock icon, 0.75rem at 700. Its tinted fills are translucent, so the badge reads the same on a Panel card, a Panel Two Kanban card and a hovered card:
...
**Financeiro.** The same 90rem cap and page padding (6rem at the bottom), 1.25rem between blocks, and the whole page is its own inline-size container, so every change below follows the content width (side menu open, collapsed or drawer), not the window. Top to bottom:
- **Header:** a three-column grid (1fr | auto | 1fr, 0.75rem by 1rem gap). The Page Title "Financeiro" sits at the left. The month selector sits in the center: two 2.5rem chevron buttons (Muted, 10px radius, Panel Two and Ink on hover; 2.75rem at 60rem and below) around the month name ("Setembro de 2026", 1.0625rem at 800, min 11.5rem, centered, announced politely). The actions sit at the right, 1.25rem apart: "Regras" and "Categorias" as link buttons, then the primary "+ Lançamento".
- **Scoreboard:** see Financeiro under Components.
- **Grid:** entries at 1.65fr and the side column at 1fr, 1.25rem gap, top-aligned. The side column stacks 1.25rem apart: Contas e cartões, Bancos conectados (administrators only), the Gastos fixos card (when any exists), Para onde foi, Resultado do mês, Últimos 6 meses.

Container steps: at 66rem and below the header puts title and actions on the first row and the month selector on a second row, left-aligned. At 58rem and below the grid becomes one column and the side column dissolves into it (`display: contents`): Contas e cartões comes first, because the balance is the first question, then the entries, then Bancos conectados, the Gastos fixos card and the summary panels; the scoreboard gains "Ver resumo do mês", a link button under the cells (0 1.5rem 0.9rem margin) that jumps to Para onde foi, and it shows only in this one-column layout. At 44rem and below the month selector spreads across the full width, the scoreboard becomes 2 by 2, the type segments take a full row and split it, and the selects share a row (9rem basis). At 30rem and below each select takes its own row, the "A revisar" row's category select drops under its entry at full width (0 0.75rem 0.6rem 2.2rem margin), and the Resultado do mês columns narrow to 2.5rem and 6.5rem. The first run (no account yet) replaces scoreboard and grid with a single panel.

**Gastos fixos.** The Financeiro page frame (90rem cap, same padding, 1.25rem between blocks, its own inline-size container), top to bottom:
- **Header:** a flex row, the Page Title "Gastos fixos" at the left and the primary "+ Gasto fixo" at the right.
- **Scoreboard:** the Financeiro scoreboard with four cells (Por mês, Assinaturas por ano, Parcelas a pagar, Pagos este mês); see Gastos fixos under Components.
- **Grid:** Próximos 30 dias at 1fr and the registrations at 1.35fr, 1.25rem gap, top-aligned; the registration panels stack 1.25rem apart.

Container steps: at 58rem and below the grid becomes one column with Próximos 30 dias first. At 30rem and below its panels pad 1.25rem and a charge row top-aligns its two sides. With no account or no registration yet, one first-run panel replaces scoreboard and grid.
...
**At 60rem and below (single column).** The side menu leaves the grid and becomes an off-canvas drawer; the top bar gains a menu button at the left and the logo centered (equal 1fr sides), keeps the avatar at the right, and wraps the level pill to a full-width second row. "+ Nova tarefa" leaves the Tarefas title row; the FAB (present at every width) remains the create action. On Financeiro "+ Lançamento" leaves the header and the page's own FAB creates an entry; on Gastos fixos "+ Gasto fixo" does the same. On Tarefas the "Filtrar" button stays in the title row (the filter panel stacks its chip groups), and the segmented filter returns to flow under the tabs. The page title drops to 1.375rem; dialog field pairs stack; the toast lifts to clear the FAB; the Painel level panel wraps its XP bar to a full-width second line.
...
Soft, friendly geometry without going bubbly. Containers (panels, dialog) round at 16px; controls (buttons, inputs, notices, toast, shell strips, account menu, user card, icon buttons, level-up banner) at 10px; interactive rows and task cards, side-menu items, menu items, chart columns on hover, the chart tooltip, the iOS step chip and small overlays at 8px. Data ends round at 4px: an XP bar (and a six-month bar on Financeiro) rounds only its top corners and a category bar (and a Para onde foi bar) only its right end, so the baseline stays square. Anything that represents a count, a state token or a filter is a full pill: the NÍVEL badge, the XP track and fill, the level pill, the ERRO tag, the segmented well and its segments (in flow too), the filter chips, the account state pills, the switch track, and the 3px active-tab bar. Circles are reserved for identity, completion, color and the floating action: the avatar, the task check, the timeline check, the category dot (and its hollow ring on Financeiro), the color swatch, the FAB. Borders are always 1px, solid except the dashed "+ Nova coluna" Kanban slot, the dashed daily-cap line and the dashed empty boxes; rings on the avatar and task check are 2px, and so is the timeline connector. Checkboxes are squares at the 4px data radius with a 2px ring, so a selection box never reads as the round completion check. The Foco cycle pills are full pills, because they count.

The icon family is drawn in-house on a 16px grid: 2px strokes (2.2px for the check), round caps and joins, no fills, color through `currentColor`. Twenty-seven glyphs exist: plus, check, pencil, list, chart, person, side-menu panel, menu, clock, stopwatch, play, pause, skip, repeat (two opposed arcs, each with a corner arrowhead), wallet, close, download, share, no connection, calendar, ellipsis, funnel, magnifier, tag, shield, chevron left and chevron right. Single-point marks (the ellipsis dots, the tag's hole and the wallet's clasp) use a 2.5px stroke so they read as dots. New icons follow the same stroke. Icons sit at 1rem in buttons and the iOS step chip, and 1.125rem in the side menu and the shell strips.
...
- **FAB:** at every width, a 56px Action Violet circle with a plus icon, fixed at the bottom-right (2rem on desktop, 1.25rem at 60rem and below; safe-area aware), carrying the FAB glow; hover brightens it, press scales it to 0.94. One tap opens the new-task dialog. At 60rem and below it replaces the Tarefas title row's "+ Nova tarefa". It appears on every page: the shell renders it everywhere except Tarefas, Financeiro and Gastos fixos, which render their own that open their dialog in place. Financeiro's FAB (new entry) and Gastos fixos' FAB (new fixed expense) show only at 60rem and below, where they replace the header's "+ Lançamento" and "+ Gasto fixo".
...
- **Money field:** the same well with the Money Entry type and a numeric keyboard, always showing a formatted value ("R$ 0,00"). Digits fill from the right like a card machine ("4590" reads R$ 45,90), and the caret stays at the end.
- **Checkbox:** a neutral 1.125rem square, 4px radius, 2px Check Ring border, no fill. Checked, it takes an Ink fill and border with a 2px Ground tick. Border and fill change over 0.15s. Selection is neutral, so a checkbox is never violet, and never green (green is the completion circle). Used in the task picker, the Foco music option and the finance account dialog.
...
- **Sections** (user request, 2026-09-13), in this order, each a group labelled by its title: **Rotina** (Tarefas, list icon; Foco, stopwatch; Painel, chart), **Finanças** (Financeiro, wallet; Gastos fixos, repeat), **Organização** (Categorias e tags, tag; Integrações, calendar) and **Conta** (Perfil, person; Administração, shield). An account sees only the items for the features it has, and a section with no items for the account is dropped; Administração appears only for the admin role. The section title uses the Label style (0.75rem, 700, 0.07em uppercase, Muted) with 0.35rem below it, and sections sit 1.125rem apart. When the sections do not fit the height, only the navigation scrolls (thin scrollbar in Rule Strong); while more items sit below, its foot fades over 3.5rem through a scroll-driven mask, which disappears at the end of the list.
- **Items:** 44px rows, 8px radius, a 1.125rem icon 0.75rem from the label, 600 weight, Muted at rest. Hover gives Panel Two and Ink. The **current page** is neutral: Panel Two fill, the Selected inset in Rule Strong, Ink text. Never violet, never a side stripe.
...
- **Collapsed (desktop):** a 4.25rem icon rail. The logo swaps for the 32px icon, the collapse button stacks under it, labels and the user card's text hide, section titles stay only for screen readers and a 1.5rem Rule hairline separates the sections, items center their icons and show their name as a native tooltip. The choice persists in `localStorage`.
...
**In-flow segments.** Inside forms and filter rows (Financeiro's entry type filter, the entry dialog's Saída / Entrada / Transferência, the expense category's group) the same well holds 2.25rem segments (2.75rem at 60rem and below) with 0 0.95rem padding, left-aligned and wrapping, without counts. The selection is `aria-pressed` and neutral, as above.
...
- **Money charts:** Financeiro's Para onde foi and Últimos 6 meses do not use green; they follow the Money Direction Rule (see Financeiro).
...
### Financeiro
The month on one page (user decision, 2026-09-13), reached from the side menu's Finanças section (wallet icon). For its grid and container steps, see Layout. Every figure is tabular, in reais formatted pt-BR, and follows the Money Direction Rule.
- **Scoreboard:** one Panel with no padding, four equal cells (Entradas, Saídas, Resultado, Poupado) divided by 1px Rules. A cell pads 1.1rem by 1.5rem (0.9rem by 1.1rem when 2 by 2) and holds its Label caption over the Stat Figure, 0.4rem apart, never wrapping. Entradas is Entrada Teal and Saídas Saída Coral; Resultado carries its sign and the hue of that sign (Ink at zero); Poupado is an Ink percentage, or "sem entradas" at 0.9375rem and 600 in Muted. In the 2 by 2 layout the Rules follow the cells. Under each figure a comparison line (0.75rem, 600, Muted, tabular) compares with the previous month: Entradas and Saídas as a signed percentage ("+19% sobre agosto"), Resultado as a difference ("R$ 88,30 a mais que em agosto"), Poupado in points ("−7 p.p. sobre agosto"), "igual a agosto" when equal. While the month is under way it compares with the previous month up to the same day ("sobre agosto até o dia 13"). With no history in the previous month there is no line; cells top-align so the figures stay on one row.
- **Filters:** one wrapping row 0.6rem apart: the search field with the magnifier inside (basis 13rem), the in-flow type segments (Todos, Entradas, Saídas, Transferências), and two selects (Todas as categorias, with "A revisar" as an option, and Todas as contas). Under the filters, while the month holds entries with no category and the "A revisar" filter is off, the **"a revisar" strip**: a 1px Rule Strong box with no fill, 10px radius, 0.7rem by 0.9rem padding, 0.5rem above, "6 lançamentos a revisar neste mês" at 0.875rem and 600 in Ink at the left and "Revisar" as a link button at the right (it sets the filter to A revisar and the type to Todos). With a filter on, a 0.875rem Muted `role="status"` line counts the entries with their inflow and outflow sums, with "Limpar filtros" as a link button at the right.
- **Entries by day:** each day is a Label heading (1rem above, 0.45rem below, 0.75rem inset) with the day ("DOM, 13 DE SET") at the left and its signed result at the right (0.8125rem at 700, in the hue of its sign; no result on a scheduled day or a day of transfers only), then the day's rows in a hairline list: Panel fill, 1px Rule border, 10px radius, no shadow, 1px Rules between rows. The list is not a card and the rows are not cards.
- **Entry row:** a button, min 3.5rem, 0.55rem by 1rem padding, 0.85rem gaps, Panel Two on hover. Three columns: the category tile (see the Money Direction Rule: hollow for an entry "A revisar", Transfer Wash with a sky dot for a transfer); the description at 700 over the detail row; and the value at 800 in its direction hue (a transfer's in sky at 700). The detail line is 0.8125rem Muted and ellipsized: the category name at 600 in its tinted color, " · ", then the account ("Lazer · Cartão Nubank"), or "Nubank → Poupança" for a transfer; for an entry "A revisar" the whole line is Ink. A bank entry can carry **marks**, "pendente" and "possível duplicata" joined by " · ", at 0.8125rem and 600 in Ink: they sit outside the ellipsis in a wrapping row, beside the detail when it fits and on their own line when it does not, so a phone never cuts them. Only inflows show "+"; outflows show "−". It opens the entry dialog.
- **"A revisar" row:** with the category filter on A revisar, each entry that is not a transfer gets an inline category select beside its row (11.5rem, 2.5rem tall, 0.875rem, 0.75rem from the right edge), offering only the categories of its type under "Escolher categoria". Choosing one categorizes the entry at once and it leaves the list.
- **Scheduled entries:** an entry dated after today (a future installment) stays in its day, but the day heading reads "DOM, 20 DE SET · AGENDADO" with no result, its description and value are Muted (the category tile and name keep their color), and it counts in neither the scoreboard, the filter sums nor the summaries.
- **Empty month:** one Muted sentence in a 1px dashed Rule Strong box (10px radius) with "Novo lançamento" as a Soft Violet text action; an empty filter shows the sentence alone.
- **Contas e cartões:** a Panel padded 1.5rem (1.25rem at 60rem and below), titled in Label style. Each account is a row button (min 3.25rem, 0.45rem by 0.75rem padding, 8px radius, Panel Two on hover): the name at 700 over its type in Meta Muted ("Cartão de crédito · vence dia 10"), and the balance at 800 at the right, Ink, or Saída Coral when negative. A card in debt shows its bill in coral as "fatura R$ 857,20", the word in Meta Muted. An account that comes from the bank adds "· banco" to its type line. Below, over a 1px Rule, "SALDO TOTAL" in Label style and the total at 1.25rem and 800; then "+ Nova conta" as a Soft Violet text action, with "n arquivadas" in Meta Muted at the right when there are any.
- **Bancos conectados:** only for the administrator (the Open Finance link is personal), after Contas e cartões, in the summary-panel recipe titled "BANCOS CONECTADOS". Each bank is a row (min 3.25rem, 0.35rem by 0.75rem padding, 1px Rules between rows): the name at 700 over a Meta Muted line ("lido hoje às 14:16 · consentimento até qui, 1 de abr"; the error sentence in Ink when the last read failed), and "Desconectar" as a link button at the right. Below, "Atualizar agora" as the outline button (min 2.75rem) and a Meta `role="status"` line with the result, in Ink on failure, out of the layout while empty. With no bank yet, one Meta sentence and the primary "Conectar bancos".
- **Desconectar dialog:** the standard Dialog: a Headline naming the bank, one dialog sentence, two radio options (keep the imported entries, or delete the entries and accounts imported from this bank), then "Desconectar" as the destructive text action at the left and "Cancelar" as a link button. No violet button: disconnecting is destructive.
- **Summary panels:** Panels padded 1.5rem with 0.85rem between parts, titled in Label style; a header row holds the title and, at the right, its legend or subtotal.
- **Gastos fixos card:** a summary Panel, 0.35rem apart: "GASTOS FIXOS", the monthly commitment ("R$ 2.804,93 por mês", 1.25rem at 800, tabular), a Meta Muted line ("3 cobranças nos próximos 7 dias") and "Ver gastos fixos" as a link button to the page. It appears only when something is registered.
- **Para onde foi:** the month's outflows by category, largest first. Under the title, one sentence at 0.875rem and 600 in Ink ("Onde mais gastou: Moradia, 62% das saídas."). Each category is a row button (0.5rem by 0.75rem padding, 8px radius, Panel Two on hover): the category tile and the name at 600 (ellipsized), the share (0.75rem, 600, Muted, right-aligned) and the value at 800 on one line; below it, a full-width 0.5rem Panel Two track (Rule on hover) holding the bar, square at the start and 4px round at the end. Every bar measures on the same full-width track, scaled to the largest category (never below 1.5%). Each bar is its category's color (Muted when it has none); "A revisar" takes the hollow tile. Tapping a row sets the category filter, resets the type to Todos and scrolls to the entries. Empty: "Nenhuma saída neste mês." in Meta Muted.
- **Resultado do mês:** the personal income statement, a hairline list: Receitas, Despesas fixas, Despesas variáveis, Assinaturas, and A revisar only when it holds something, then Resultado. A row is a three-column grid with fixed columns (the label, the share in 2.75rem, the value in 7.25rem), so shares and values align down the list like a statement; min 2.75rem, 0.35rem by 0.75rem padding, 8px radius, 1px Rules between rows. The label is 600; the share is 0.75rem, 600, Muted, a percentage of the month's receitas (none on Receitas); the value is 800, signed (Receitas "+", expenses "−") and in the hue of its sign. A row with categories is a button (Panel Two on hover, `aria-expanded`) led by a Muted 2px chevron that turns from right to down; it opens its categories beneath, indented 2rem, each a 2rem line at 0.875rem in Muted with a 0.5rem dot and the value. Rows without categories indent their label to align with the chevron rows. The Resultado row sits over a Rule Strong divider, its label at 800 and its value at 1.125rem.
- **Últimos 6 meses:** entries against exits for the six months ending at the chosen month. The header carries a legend at the right (0.75rem, 600, Muted: a 0.625rem key with 2px corners per series). A 9rem plotting area over a 1px Rule baseline holds one column per month, 0.35rem apart; each column is a button (8px top corners, Panel Two on hover and when chosen, `aria-pressed`) holding two bars 2px apart, min(1rem, 40%) wide with 4px top corners, scaled to the largest value of either series: **Entradas** in Entrada Teal at the left, **Saídas** in Saída Coral at the right; the legend keys take the same hues. Short month names run beneath (0.75rem Muted; the chosen month Ink at 700). No bar is labeled; hovering, focusing or tapping a column chooses it, and its figures show as text in the figcaption, announced politely: the month name at 0.875rem in Ink on its own line, then "Entradas R$ …", "Saídas R$ …" and "Resultado +R$ …" at 0.8125rem Muted. Each column's label also carries its figures.
- **First run:** with no account yet, a Panel (max 36rem, 1.75rem padding) holds a 1.25rem/800 title "Comece pela sua primeira conta", one Muted sentence and the primary "+ Cadastrar conta". The header keeps "Regras" and "Categorias" but drops "+ Lançamento", and there is no FAB.
- **Entry dialog:** the standard Dialog. The in-flow segments choose Saída, Entrada or Transferência; then Valor (money field) and Data as a pair; Descrição; Categoria, only the categories of the chosen type (absent for a transfer); the account ("Saiu de", "Entrou em", or "De" and "Para" as a pair). A Meta hint explains that paying a card bill is a transfer. Editing adds "Excluir lançamento" as the destructive text action; deleting shows the undo toast "Lançamento excluído.". **A bank entry is locked:** a Meta hint ("Veio do banco: valor, data e conta seguem o que o banco informa…") takes the place of the segments, and Valor, Data and the account fields are gone, leaving Descrição and Categoria. When the bank entry may repeat one typed by hand, the **duplicate notice** opens the dialog above the form: a neutral box on Ground (1px Rule Strong border, 10px radius, 0.9rem by 1rem padding, 1rem below) with its title at 700, a Meta sentence naming the hand-typed entry, then the outline "Juntar (fica o do banco)" and "São diferentes" as a link button. No alarm tint. After a new category is saved on a bank entry, the rule suggestion toast appears (see Toast).
- **Account dialog:** the standard Dialog. Nome and Tipo as a pair, then the money field (Saldo atual, or Fatura em aberto for a card) with a Meta hint, the neutral checkbox "Saldo negativo (cheque especial)" or, for a card, Fecha dia and Vence dia as a pair of selects. Editing adds the "Arquivar conta" checkbox and "Excluir conta"; an account with entries can't be deleted, and the notice says to archive it. An account from the bank shows a Meta hint in place of the money field, and when accounts typed by hand look like it, the **Juntar contas box**: the duplicate notice's neutral box (no margin), "Já tinha esta conta cadastrada à mão?", a Meta sentence, then a select and the outline "Juntar contas" on one wrapping row.
- **Categorias dialog:** one Dialog that swaps two views in place. The list view: one Muted sentence, then DESPESAS and RECEITAS, each in the Categorias e tags row pattern (8px dot, name at 600, the group in Meta Muted, "Editar") with "+ Nova despesa" or "+ Nova receita" below, and one primary "Fechar". The edit view takes the dialog title: Nome, "Grupo no resumo" as in-flow segments (Fixa, Variável, Assinatura; expenses only) and the color picker, with "Voltar" in place of Cancelar. Deleting a category in use needs a second press ("Excluir mesmo assim"); its entries become "A revisar".
- **Regras dialog:** "Regras de categoria" in the standard Dialog, opened from the header's "Regras". One dialog sentence, then the create form closed by a 1px Rule (1.25rem above and below): "Descrição contém" with a Meta hint, "Categoria" (grouped Despesas and Receitas) and "Vale para" as a pair, a Meta `role="status"` line, then the primary "Criar regra" and "Aplicar regras agora" as a link button on one row. Below, the rules in a hairline list that scrolls past 18rem: each row (min 2.75rem) holds the term at 700 and a Meta "→ Moradia · sugerida", with "Excluir" as a link button at the right; with none, "Nenhuma regra ainda." in Meta. The only footer action is the outline "Fechar". Opened from the suggestion toast, the form arrives filled.

### Gastos fixos
Subscriptions, installment purchases, fixed bills and other repeating costs on their own page (user decision, 2026-09-13), reached from the side menu's Finanças section (repeat icon) and from the Gastos fixos card on Financeiro. For its grid and container steps, see Layout. It is Financeiro's parts reused and follows the Money Direction Rule: every value is an outflow, so charge and registration values are Saída Coral, and rows lead with the category tile (hollow with no category). The scoreboard and the subtotals stay Ink, and the deadline tints stay inside the Deadline Badge.
- **Scoreboard:** the Financeiro scoreboard: Por mês (with the comparison-line style carrying "o que já está comprometido"), Assinaturas por ano, Parcelas a pagar ("em 2 compras" beneath), Pagos este mês ("2 de 5").
- **Próximos 30 dias:** a Panel padded 1.5rem, titled in Label style. Its groups, each under a Label heading (0.5rem above, 0.35rem below): **Vencidas** first (unpaid charges already due), then one group per day ("QUA, 16 DE SET"), then **Pagas este mês** (charges already paid whose day has passed). A charge row is not a button: min 3.25rem, 0.45rem vertical padding, 1px Rules between rows in a group. At the left, the category tile, then the name at 700 over the detail line (0.8125rem Muted: "Conta fixa · valor aproximado · Nubank", "parcela 2 de 6 · Cartão Nubank"), both ellipsized. At the right, the value at 800 over a state line (0.8125rem, 0.6rem gaps): unpaid, the task Deadline Badge ("atrasada · 16 ago", "16 set") and "Pagar" as a link button; paid, "paga" at 700 in Muted with a 0.75rem check, then "Desfazer", while name and value (coral until then) turn Muted; an installment, "lançada" in Meta Muted with no action, because its entries already exist. Paying a fixed value records it today and shows the undo toast "Conta de luz marcada como paga."; a variable value opens the Pagar dialog. Empty: "Nada vence nos próximos 30 dias." in Meta Muted.
- **Registrations by type:** one Panel per type that has items (Assinaturas, Compras parceladas, Contas fixas, Outros), padded 1.5rem, the Label title at the left, led by a 0.5rem type dot in a fixed color (Assinatura #b39ddb, Parcelada #6c9be8, Conta fixa #e0a050, Outro #8f9bb3), and the monthly subtotal at the right ("R$ 98,53/mês", 0.875rem at 800, tabular). Each item is a row button in the account-row recipe (min 3.25rem, 0.45rem by 0.75rem padding, 8px radius, Panel Two on hover), active items first: the category tile, the name at 700 over the detail in Meta Muted ("todo dia 6 · Cartão Nubank", "todo ano em 3 nov", "7 de 12 · faltam R$ 2.082,50"), and at the right the value at 800 in coral over an extra line (0.75rem, 600, Muted: "R$ 670,80/ano" for a subscription, "próxima: seg, 5 de out" for a bill, the card for an installment, "pausada" or "quitada"). A paused or paid-off item stays listed with its name and value in Muted. It opens the Gasto fixo dialog.
- **First run:** the Financeiro first-run panel, "Nenhum gasto fixo ainda", one Muted sentence and the primary "+ Novo gasto fixo"; with no account, the sentence asks for one first and the primary is "Ir para o Financeiro".
- **Gasto fixo dialog:** the standard Dialog, "Novo gasto fixo" or "Editar gasto fixo". The in-flow segments choose Assinatura, Parcelada, Conta fixa or Outro (an existing installment purchase shows no segments; Parcelada is not offered when editing). Nome, with an example for the type ("Ex.: Netflix"). Valor (Valor da parcela for an installment) paired with Parcelas (a 2x to 72x select) or Repete (Todo mês, Todo ano, Toda semana); for an installment with a value, a Meta hint tucked under the pair gives the total ("12 × R$ 250,00 = R$ 3.000,00"). Primeira cobrança (Primeira parcela) paired with "Termina em (opcional)", with a Meta hint below. Categoria (following the type until touched; "A revisar" is an option) paired with "Cobrado em". Neutral checkboxes: "O valor muda a cada mês (confirmo ao pagar)" and, when editing, "Pausar". Editing adds "Excluir gasto fixo" as the destructive text action; the first press shows a notice on what happens to past entries and the action becomes "Excluir mesmo assim".
- **Pagar dialog:** for a charge whose value varies. Title "Pagar Conta de luz", one dialog sentence ("Cobrança de qua, 16 de set. Confirme o valor e a data do pagamento."), Valor pago (focused) and Pago em as a pair, Pago com, then Cancelar and the primary "Marcar como paga".
...
### Toast
Fixed, centered 1.5rem above the bottom (above the FAB on mobile; on Financeiro with the "A revisar" filter on, at 60rem and up it moves to the bottom-right, 2rem from the edge, so it never covers the next row's category select), Panel Two fill, 1px Rule Strong border, 10px radius, Panel lift, 0.875rem at 600. Enters with a 12px rise. One toast at a time. Announced politely (`role="status"`, or `alert` for errors). Four variants:
- **Undo:** "Tarefa excluída" with a "Desfazer" link button and a 2px Rule Strong time bar along the bottom edge that shrinks to zero over 5s. The undo toast can carry its own text and action: ending a focus session shows "Sessão encerrada." with Desfazer. That Desfazer restores the session as it was, unless a new session has begun, and the toast closes after 5s. The action's text can change too: after a category is chosen for a bank entry, "Categoria salva. Criar uma regra para PADARIA?" carries "Criar regra", which opens the Regras dialog with the rule filled in; Financeiro closes that toast after 8s.
...
- **Do** show category color as an 8px dot next to the category name; the task card's cover band and the color picker are the only fills, except on the money pages, where the category tile, the tinted name and the Para onde foi bars carry it.
...
- **Do** draw charts in single-hue green with 4px rounded data ends, a neutral scale reference, one direct label on the extreme value, a hover tooltip, and a visually hidden table with the same data. Money charts are the exception: Para onde foi bars take their category color, and Últimos 6 meses shows Entradas in teal and Saídas in coral with a legend, per the Money Direction Rule.
...
- **Do** show money as signed tabular figures in its direction hue (Entrada teal, Saída coral, Transferência sky), drop a scheduled value to Muted, and mark an uncategorized entry with a hollow tile and "A revisar".
- **Do** list a future-dated entry as "agendado" in Muted and leave it out of the scoreboard, filter sums and summaries.
- **Do** keep a paused or paid-off fixed expense in its list, with name and value in Muted.

### Don't:
- **Don't** add a third accent, or use red for errors, deletion or a streak at risk. The amber and rose deadline tints stay inside the Deadline Badge and the deadline reminder, and the money direction hues stay on money figures, money charts and the transfer tile.
- **Don't** use purple, a colored side stripe, or the category color to mark a selected item or the current page.
- **Don't** fill, border or tint surfaces or chart bars with category colors outside the money pages (their category tile and Para onde foi bars). The one task fill exception is the task card's category-colored cover band (user override, 2026-09-11), faded to 40% on completed cards; the card itself is a flat Panel body (Panel Two in Kanban), never a shadowed nested card.
...
- **Don't** label every bar, add gridlines, or use more than one hue for magnitude. Para onde foi's category colors name the category; length alone carries the amount.
...
- **Don't** color money with Level Green or Deadline Rose, show a direction hue without its sign, or tint a month-over-month change; the comparison stays a Muted sentence.
