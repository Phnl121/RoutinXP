...
**The Dot-Only Category Rule.** User-chosen category colors are data. They appear as an 8px round dot beside the category name (rail, chips, row category line, Lista group headings, the Painel's category bars and timeline items), never as fills, text color, bars or backgrounds, and never without the name nearby. Two exceptions: the color picker, where the swatch itself is the choice and fills a 36px circle; and the task row's 1px outline in its category color on all four sides (user request, 2026-09-11), which marks where one task ends and the next begins, in Lista and Quadro alike. On completed rows the outline fades to 40% of the category color, receding with the rest of the row. **Tags** have their own shape, always rectangular, so a tag is never read as a category (categories are always round dots). On a task row tags are **labels, Trello-style** (user request, 2026-09-11): a row of 4px-radius labels above the title. Named mode: a 1.375rem pill, 0 0.5rem padding, 0.75rem at 700 in Ink on the tag color mixed 48% into Panel (55% on hover, the most that keeps Ink above 4.5:1 on all eight colors), ellipsized at 12rem. Color-only mode: a 2.5rem by 0.5rem bar in the full tag color. Clicking any label switches every label between the two modes (saved per browser); it never opens the task. Completed rows show labels at 55% opacity. In the task dialog and the Perfil list a tag is a 0.75rem swatch with 3px corners beside its name.
...
**At 60rem and below (single column).** The side menu leaves the grid and becomes an off-canvas drawer; the top bar gains a menu button at the left and the logo centered (equal 1fr sides), keeps the avatar at the right, and wraps the level pill to a full-width second row. "+ Nova tarefa" leaves the Tarefas title row; the FAB (present at every width) remains the create action. On Tarefas the rail becomes a horizontally scrolling chip row that bleeds to both screen edges (negative margin equal to the page padding, scrollbar hidden), and the segmented filter returns to flow under the tabs. The page title drops to 1.5rem; dialog field pairs stack; the toast lifts to clear the FAB; the Painel level panel wraps its XP bar to a full-width second line.
...
### List Rows (tasks)
Each row is a card outlined by a **1px border in its category color on all four sides** (Rule when the category is unknown), 8px radius, no shadow, 0.25rem left padding; rows sit 0.5rem apart inside the group panel. The hover wash fills the card. A row is a three-column grid:
...
- **Don't** fill, border or tint surfaces or chart bars with category colors. The one border exception is the task row's 1px category-colored outline (user override, 2026-09-11), faded to 40% on completed rows; it is a border only, never a shadowed nested card.
